...
const Portrait = ({ disabled = false, name = 'home', className }) => {
    ...
    return (
        <Link to={`/${name}`} onMouseEnter={handleHover} onMouseLeave={handleHover} className={`relative h-full transition-all ${!disabled ? 'hover:filter-none hover:scale-[105%] filter grayscale' : ''} bg-cover bg-center bg-no-repeat ${className}`} style={{backgroundImage: `url(${name == 'register' ? PortraitOne : name == 'login' ? PortraitTwo : PortraitThree})`}}>
