...
const Card = ({ medium, className }) => {
    return (
        <div className={`relative w-full h-64 bg-contain bg-center bg-no-repeat flex flex-col p-5`} style={{backgroundImage: `url(${Shadow})`}}>
            <div className="relative w-full h-full bg-cover bg-center bg-no-repeat" style={{backgroundImage: `url(${medium.image ? 'http://localhost/storage/' . medium.image.path : Background})`}}>
