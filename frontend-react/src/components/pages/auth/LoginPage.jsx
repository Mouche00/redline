...
const Portrait = ({ name = 'home' }) => {
    ...
    return (
        <Link to={`/${name}`} onMouseEnter={handleHover} onMouseLeave={handleHover} className="relative h-full w-full transition-all hover:filter-none hover:scale-[105%] filter grayscale bg-cover bg-center bg-no-repeat" style={{backgroundImage: `url(${name == 'register' ? PortraitThree : PortraitOne})`}}>
