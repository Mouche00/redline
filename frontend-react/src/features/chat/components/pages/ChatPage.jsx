    ...
    return (
        <section className="relative w-full h-screen bg-grey flex items-center justify-center">
            <div className="absolute top-0 left-0 w-[25%] translate-x-8 z-10 h-full flex">
                ...
                <div className="bg-contain bg-center bg-repeat w-full h-full p-4 space-y-4" style={{backgroundImage: `url(${Texture})`}}>
                    <div className="flex">
                        ...
                        <>
                            ...
                            <div className="h-full w-full border-4 border-white">
                                    ...
                                    <div className="w-full flex items-center p-4">
                                        <div className={`w-full flex flex-col gap-8`}>
                                            {messages.map((message, index) => (
                                                message.message ? (
                                                    <div key={index} className={`flex flex-col ${message.user_id == JSON.parse(user).id ? 'items-end' : 'items-start'}`}>
                                                        <div className={`${message.user_id == JSON.parse(user).id ? 'text-end' : 'text-start'} relative z-10 min-w-4 text-white font-black h-fit text-xl`}>
                                ...
                                <form onSubmit={handleSubmit} className={`flex w-full h-[8%]`}>
                                    ...
                                    <div className="w-[80%]">
                                        <input onChange={handleChange} type="text" name='message' className="w-full h-full border-4 outline-none border-black p-2 bg-white font-black h-fit mx-[-2px]" />
